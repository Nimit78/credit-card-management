#include<bits/stdc++.h> 
 ...
 // Transaction Function 
    void add_trans(string iuserid, double amt, double balance)
     ...
     void completetransaction(){ 
         ...
         cout << "\nLogin to the portal using your User ID as the login ID :" << endl; 
         cin >> iuserid; 
 ...
 }; //linked list used for password and user id
  struct node
{
	string ids;
	int password;
 ...
 { 
	...
	while(ch==1)
	{
	 ...
	 cout<<"\nclick 1 for add next id and click 0 for next step: ";
	 cin>>ch;
	}
    string tree;
    int pass,che;
    ...
    while(temp!=0)
	{
        if(temp->ids==tree && temp->password==pass){
    ...
    if(che==0)
    {
     cout<<"\nLogin failed: \n Please enter right id and password: ";
     goto er;
    }
     int op; 
     do 
